...
st.title("🎬 AI Short Video Generator — Fire Show Preset")
st.write("Horizontal clean cinematic preset for fire juggling videos. Optimized for large compressed files and downloadable MP4 export.")


with st.sidebar:
    st.header("Video Settings")

    target_length = st.slider("Target length seconds", 10, 90, 25)
    use_smart = st.checkbox("Smart motion-based cutting", value=True)
    vertical_mode = st.checkbox("Vertical 9:16 crop", value=False, help="Keep OFF for horizontal Facebook / event videos.")

    st.header("Visual Settings")
    logo_size = st.slider("Logo size", 100, 700, 260)
    title_size = st.slider("Title size", 30, 120, 58)
    caption_size = st.slider("Caption size", 30, 110, 54)
    ...
    st.header("Advanced")
    motion_samples = st.slider("Motion samples per video", 6, 40, 14, help="Lower values are safer for long videos.")
    selected_moments = st.slider("Selected moments per video", 3, 12, 7)
    max_file_mb = st.slider("Large file warning threshold MB", 100, 2500, 1200, step=100)
    auto_optimize = st.checkbox("Auto-optimize large uploads before editing", value=True)
    optimize_width = st.select_slider("Optimize width", options=[480, 640, 720, 960, 1280], value=640)
    optimize_crf = st.slider("Optimize compression CRF", 28, 38, 35, help="Higher = smaller file, lower quality.")
...
title = st.text_input("Title", "🔥 Tűz. Mozgás. Kapcsolat.")
organization = st.text_input("Organization", "Mandala of Fire × EUFlowria")
...
def normalize_video(input_path, output_path):
    """Normalize and lightly compress so long phone videos do not crash MoviePy/Streamlit."""
    ffmpeg = imageio_ffmpeg.get_ffmpeg_exe()

    if auto_optimize:
        vf = f"scale='min({int(optimize_width)},iw)':-2,fps=24,setsar=1"
        crf_value = str(int(optimize_crf))
...
if uploaded_videos:
    ...
    if st.button("Generate Short"):
        ...
        if large_files and not auto_optimize:
            st.error(
                "Please compress the large files before generating, or enable auto-optimization. "
                "This prevents Streamlit/MoviePy crashes."
            )
            st.stop()
        elif large_files and auto_optimize:
            st.warning("Large files detected, but auto-optimization is ON. The app will compress them first.")

        temp_dir = Path(tempfile.mkdtemp())
        output_path = temp_dir / "zsonglor_horizontal_clean_cinematic.mp4"

        processed_clips = []
        per_video_target = max(3.0, float(target_length) / len(uploaded_videos))

        with st.spinner("Optimizing and processing videos... This can take several minutes for long files."):
        ...
        with open(output_path, "rb") as handle:
            st.download_button(
                ...
                handle,
                "zsonglor_horizontal_clean_cinematic.mp4",
                "video/mp4",
            )
else:
